...
//				.createCriteria(User.class).list();
	...
	public int userLitmit(String tvlKey) {

		try {
			String hql = "SELECT TBL_VALUE FROM TBL_KEY_VALUE where TBL_KEY =:TBL_KEY";
			Query query = sessionFactory.getCurrentSession().createQuery(hql);
			query.setParameter("TBL_KEY", tvlKey);
			return ((Number) query.uniqueResult()).intValue();
		} catch (Exception e) {
			System.out.println("Exception===" + e);
		}
	...
	public MLIExposureId getExposureId(String mem_bnk_id,String fyBasedOnStartAndEndDate)
		...
		MLIExposureId mliExposureId = (MLIExposureId) sessionFactory
				...
				.add(Restrictions.eq("MEM_BNK_ID", mem_bnk_id)).uniqueResult();
...
//		}
	...
	public void updateUserInfo(UserInfoModel userInfo) {
		...
		String isActive=userInfo.getIsActive();
		String userid=userInfo.getUsr_id();
		String phone_code=userInfo.getPhone_code();
		Session session = sessionFactory.openSession();
		String hql = "UPDATE UserInfoModel set mobileNumber =:mobileNumber1,phoneNumber=:phoneNumber1,email=:email1,userType=:userType1,phone_code=:phone_code,isActive=:isActive1 WHERE usr_id = :user_id1"; 
		...
		query.setParameter("phone_code", phone_code);
		query.setParameter("isActive1",isActive);
...
//		}
	...
	public List<BulkUploadTracker> getBulkUploadTrack(String userId) {
		return (List<BulkUploadTracker>) sessionFactory.getCurrentSession().createCriteria(BulkUploadTracker.class).add(Restrictions.eq("upload_by", userId)).addOrder(Order.desc("upload_date")).list();
	}
	
	public void updateUserPrivlige(String userId,int prvId) {
		// TODO Auto-generated method stub
		Session session = sessionFactory.openSession();
		String hql = "UPDATE NBFCUserPerivilege set prvId =:prvId1 WHERE usr_id = :user_id1"; 
		          
		Query query = session.createQuery(hql);
		query.setParameter("prvId1",prvId);
		query.setParameter("user_id1",userId);
		System.out.println("Query--------: " + query);
		int result = query.executeUpdate();
		System.out.println("Rows affected: " + result);
	...
	@Override
	public void updateUserPrivlage(String userId, String roleName) {
		// TODO Auto-generated method stub
